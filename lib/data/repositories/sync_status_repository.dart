...
class SyncStatusRepository {
  ...
  Future<void> updateLastSyncTime(String entityType, DateTime syncTime) async {
    final db = await _appDatabase.database;

    // Criar um ID especial para o registro de última sincronização global
    String entityId = 'last_sync_${entityType}';

    final syncStatus = await getByEntityId(entityId, entityType);
    if (syncStatus != null) {
      final updated = syncStatus.copyWith(lastSyncTime: syncTime);
      await insertOrUpdate(updated);
    } else {
      // Criar um novo registro de status se não existir
      final newStatus = SyncStatus(
